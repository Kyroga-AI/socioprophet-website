...
`/products/noetica/beta` is a link-only landing page (not in the nav): the waitlist for the upcoming **free, self-managed edition** of Noetica, for individuals and small businesses — the QR target from Gus's MPOWER Cumbre 2026 talk. Positioning: "Stop renting AI. Start building a brain you own & don't give it away." Frame the segment as next in line, never as "not for you yet" (Gus's explicit direction). Noetica runs today as managed deployments for organisations. Keep it English, plain, and short, and keep claims qualified ("no AI bills" holds only for local models; "your IP stays on your machine" rather than absolute "no leakage"). It has its own static `<head>` for link previews (see Deploy).
